...
# 环境搭建
- anaconda+pycharm
- anaconda使用（windows电脑环境上直接打开Anaconde prompt）
    - conda list: 显示当前环境安装的包
    - conda env list:显示安装的虚拟环境列表
    - conda create -n env_name python=3.6：创建一个新的虚拟环境
    - 激活conda的虚拟环境
        - source activate env_name(Linux)
        - conda activate env_name(win) 
    - 安装django1.8：pip install django==1.8
 
 
# 后台需要的流程
- 1. HTTP 接受请求（理解请求要求,解剖http请求）
- 2. 拿到请求之后，处理相关的业务
- 3. 反馈结果
# 创建第一个django程序
- 命令行启动
        ...
        cd tulingxueyuan
        python manage.py runserver（启动django命令）
        
- pycharm 启动
    - 需要配置
        1.File -> Settings -> project -> project interpreter -> Existing environment -> 选择新的虚拟环境路径 -> 点击ok ：这样就可以在新的虚拟环境上用新的python环境了
        2.选中manage.py文件 -> 选中右上角manage下拉框，选中Edit，出现设置框 -> Parameters栏中输入runserver命令
        3.再选中manage.py文件debug就可以直接pycharm启动django
    
# 路由系统-urls
- 创建app
    - app：负责一个具体业务或者一类具体业务的模块
    - python manage.py startapp teacher (创建一个新功能模块)
...
- url匹配规则
    ...
    - 子url一旦被调用，则不会返回到主url，如果在子里面没有找到相应的方法就直接返回报错信息。
        - `/one/two/three/`
    - 正则以r（r：不需要转义）开头,表示不需要转义，注意尖号(^：开头一定是什么什么)和美元符号($：末尾一定是什么什么)
        - `/one/two/three` 配对 r'^one/ '   （路由屁匹配的时候会自动省略最前面的斜杠/）
        - `/oo/one/two/three` 不配对 r'^one/'
        - `/one/two/three/` 配对 r'three/$'
        - `/oo/one/two/three/oo/` 不配对 r'three/$'
        - 开头不需要有反斜杠
    - 如果从上向下都没有找到合适的匹配内容，则报错error
...
- 1:1 OneToOne
    - 建立关系：在模型任意一边即可，使用OneToOneField
        代码：my_school = models.OneToOneField(School)
        例如：tlxy_db->rlt->models.py
    - add:  
...
- 步骤：
    ...
    2. 在tmepltes文件夹下编写模并调用,所有的模板html文件都在当前文件夹下，如果实际项目中有多个app功能的话，多建几个文件夹用来区分app。
...
- 创建超级用户
    1.为迁移数据库做准备：python manage.py makemigrations
    2.迁移数据库：python manage.py migrate
    3.创建超级管理人：python manege.py createsuperuser
- 配置settings文件
    LANGUAGE_CODE = 'zh-Hans'  -》设置后台管理平台为中文
...
- 实现方式
    - ModelAdmin
    - 装饰器，案例：myAdmin-》aadmin-》admin.py
